...
def interface_app_login(account, password):
    loginurl = config.URL+"api/system/app/user/loginByAccount"
    # 请求头根据自己接口文档进行调整
    ...
    r = requests.get(loginurl, params=s, headers=headers)
    e = r.text
    resp_obj = json.loads(e)
    print(resp_obj)
    print(type(resp_obj))
    token = resp_obj["data"]["token"]["token"]
    return token


def interface_web_login():
...
if __name__ == '__main__':
    a=interface_app_login("zhong", "123456")
    print(a)
